...
This file is read by any AI coding assistant working in this repository —
agents that follow the [agents.md](https://agents.md) spec (OpenAI Codex,
...
rapid-cdn has full SlapOS instance deployment tests in `software/rapid-cdn/test/test.py`. Source the SR-testing environment and invoke the test class/method:

```bash
source <slapos-sr-testing-environment>
cd software/rapid-cdn/test/
python_for_test -m unittest test.TestClassName.test_method_name
```

Useful flags: `--save-data` (regenerate expected output), `--debug` (keep instances alive on failure).
...
This sets `PATH`, `SLAPOS_CONFIGURATION`, and `SLAPOS_CLIENT_CONFIGURATION`.
