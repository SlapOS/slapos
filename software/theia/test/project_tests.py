...
class TestTheiaResilienceERP5(ERP5Mixin, test_resiliency.TestTheiaResilience):
  ...
  def _prepareExport(self):
    ...
    mariadb_backup_dump = os.path.join(mariadb_partition, 'srv', 'backup', 'mariadb-full', 'most-recent.sql.gz')
    self.assertTrue(os.path.exists(mariadb_backup_dump))
...
class TestTheiaResilienceGitlab(TestTheiaResilienceWithShortPaths):
  test_instance_max_retries = 70  # puma takes time to be ready
  backup_max_tries = 480
  backup_wait_interval = 60
