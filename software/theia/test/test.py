...
class ReRequestMixin(object):
  def rerequest(self, parameter_dict, state='started'):
...
class TestTheiaWithSR(TheiaTestCase, ReRequestMixin):
  ...
  def test(self):
    ...
    self.assertFalse(subprocess.call((slapos, 'service', 'info', old_instance_name)))
    ...
    self.assertTrue(subprocess.call((slapos, 'service', 'info', old_instance_name)))
    self.assertFalse(subprocess.call((slapos, 'service', 'info', instance_name)))
