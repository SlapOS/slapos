...
class TestTheiaResilienceERP5(TestTheiaResilience):
  ...
  def _doBackup(self):
    super(TestTheiaResilienceERP5, self)._doBackup()

    # Manually restore mariadb from backup
    mariadb_restore_script = self._getTypePartitionPath(
      'import', 'srv', 'runner', 'instance', 'slappart3', 'bin', 'restore-from-backup')
