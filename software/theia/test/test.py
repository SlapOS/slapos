...
class TheiaTestCase(SlapOSInstanceTestCase):
  ...
  instance_max_retry = 20 # same as the default
...
class TestTheiaEnv(TheiaTestCase):
  ...
  def test_theia_env(self):
    # The path of the env.log file expected to be generated by building the dummy software release
    envlog_path = os.path.join(self.computer_partition_root_path, 'srv', 'runner', 'software', 'env.log')
    ...
    # Supply the dummy software rekease
    theia_shell_process.sendline('slapos supply %s slaprunner' % self.dummy_software_path)
    theia_shell_process.expect('Requesting software installation of %s...' % self.dummy_software_path)
    ...
    # Get the theia shell envlog
    with open(envlog_path, 'r') as f:
      theia_shell_envlog = f.read()

    # Remove the env.log file to be sure a new one will be generated
    os.remove(envlog_path)
    ...
    with getSupervisorRPC(embedded_supervisord_socket_path) as embedded_supervisor:
      ...
      while embedded_supervisor.getProcessInfo('slapos-node-software')['stop'] == previous_stop_time:
        time.sleep(10)

    # Get the supervisord envlog
    with open(envlog_path, 'r') as f:
      embedded_supervisord_envlog = f.read()

    # Compare both envlogs
    self.maxDiff = None
    self.assertEqual(
      theia_shell_envlog.replace(':', '\n'),
      embedded_supervisord_envlog.replace(':', '\n'),
    )
