...
from theia_common import copytree, copydb, hashwalk, parse_installed
...
class TheiaImport(object):
  ...
  def verify(self, signaturefile):
    ...
    with open(signaturefile + '.proof', 'w') as f:
      for s in hashwalk(self.backup_dir, mirror_partitions):
        f.write(s + '\n')
    diffcommand = ('diff', signaturefile, signaturefile + '.proof')
    print(' '.join(diffcommand))
    if sp.call(diffcommand):
      msg = 'ERROR the backup signatures do not match'
      self.loginfo(msg)
      raise Exception(msg)
  ...
  def restore(self):
    ...
    self.verify(os.path.join(self.backup_dir, os.pardir, 'backup.signature'))
