...
class ZopeSkinsMixin:
  ...
  def _getAuthenticatedZopeUrl(cls, path, family_name='default'):
    ...
    return urllib_parse.urljoin(base_url, path)
...
class ZopeTestMixin(ZopeSkinsMixin, CrontabMixin):
  ...
  def _setUpClass(cls):
    ...
    cls.zope_verify_activity_processing_url = urllib_parse.urljoin(
        cls.zope_base_url,
    ...
    cls.zope_log_message_url = urllib_parse.urljoin(
        cls.zope_base_url,
        'ERP5Site_logMessage',
    )
    ...
    cls.zope_long_request_url = urllib_parse.urljoin(
        cls.zope_base_url,
...
class TestZopeWSGI(ZopeTestMixin, ERP5InstanceTestCase):
  ...
  def test_long_request_log_rotation(self):
    super().test_long_request_log_rotation(self)

  @unittest.expectedFailure
  def test_basic_authentication_user_in_access_log(self):
    super().test_basic_authentication_user_in_access_log(self)
