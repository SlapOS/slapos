...
class TestLog(BalancerTestCase, CrontabMixin):
  ...
  def test_access_log_apachedex_report(self) -> None:
    ...
    # yesterday
    requests.get(self.default_balancer_zope_url + 'error-two-days-ago', verify=False)
    self._executeCrontabAtDate('logrotate', 'yesterday 00:00')
    requests.get(self.default_balancer_zope_url + 'error-yesterday', verify=False)
    ...
    # because we run apachedex with error details, we can see our 404 requests
