...
LineChart.prototype = {
  ...
  prepareData: function () {
    'use strict';
    var i;
    for (i = 0; i < this.data_length; i += 1) {
  ...
  draw: function () {
    ...
    this.drawGrid();
    var i, coordinates = [];
    for (i = 0; i < this.data_length - 1; i += 1) {
  ...
  drawBorder: function () {
    ...
    this.drawLine([{x: this.width, y: 0}, {x: this.width, y: this.height}],
                  '#000', 0);
    this.drawLine([{x: this.width, y: this.height}, {x: 0, y: this.height}],
                  '#000', 0);
  ...
  drawGrid: function () {
    ...
    // Horizontal lines
    var i;
    for (i = 0; i <= this.y_grid_steps; i += 1) {
      this.drawLine([
        {x: this.x_padding / 2, y: this.y_step * i + this.y_padding },
        {x: this.width - this.x_padding / 2,
    ...
    for (i = 0; i <= this.data_length; i += 1) {
      this.drawLine([
        ...
        { x: this.x_step * i + this.x_padding,
          y: this.height - this.y_padding }
      ],
        '#DDD', 5);
    }
  ...
  drawPoint: function (coordinate, style) {
    'use strict';
    this.context.beginPath();
    this.context.arc(coordinate.x, coordinate.y,
                     this.point_radius, 0, 2 * Math.PI, false);
  ...
  calcCoordinateY: function (index) {
    ...
    var proportion = this.data[index] / this.data_max,
      top_without_padding = proportion * (this.height - this.y_padding * 2);
  ...
  getMax: function () {
    'use strict';
    var i, max = this.data[0];
    for (i = 1; i < this.data_length; i += 1) {
      if (this.data[i] > max) {
        max = this.data[i];
      }
    }
  ...
  getLabelY: function (index) {
    'use strict';
    return ((this.data_max / this.y_grid_steps) * (this.y_grid_steps - index))
      .toFixed(1).replace(/[.,]0$/, "");
  },
  ...
  getPointSelected: function (x, y) {
    'use strict';
    var i, coordinates, dist;
    for (i = 0; i < this.data_length; i += 1) {
      coordinates = [
        {x: x, y: y},
        {x: this.coordinates[i].x, y: this.coordinates[i].y}
      ];
      dist = this.distanceBetweenTwoPoints(coordinates);
  ...
  clickEvent: function (event) {
    ...
    if (point !== null) {
      this.showTooltip(this.tooltip[point],
