...
class SlaprunnerTestCase(SlapOSInstanceTestCase):
  ...
  def _doTakeover(self, scope="runner-1"):
    ...
    resp = requests.get(
      ...
      verify=True)
    self.assertEqual(requests.codes.ok, resp.status_code)
    self.assertFalse("Error" in resp.text,
            "An Error occured: %s" % resp.text)
    self.assertTrue("Success" in resp.text,
...
class TestResilientDummyInstance(SlaprunnerTestCase):
  ...
  def test_basic_resilience(self):
    ...
    self.assertTrue(result_after.startswith("Hello"),
      "%s didn't contains the good content" % result_after)

    self.assertTrue(result in result_after,
