...
class EdgeSlaveMixin(MonitorTestMixin):
  ...
  def initiateSurykatkaRun(self):
    """
    Runs instance and force-runs surykatka-report-json to not wait for cron"""
    try:
    ...
    self.assertEqual(
      0,
      os.system(
        os.path.join(self.bot_partition_path, 'bin', 'surykatka-status-json'))
    )
