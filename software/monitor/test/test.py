...
class TestEdgeRegionDestroyed(TestEdgeRegion):
  ...
  def test(self):
    ...
    # avoid @classmethod used for request to re-request the instance
    ...
    self.slap.request(
        ...
        partition_parameter_kw={'_': json.dumps(self.instance_parameter_dict)},
        state='started')

    # give time to stabilise the tree
    self.slap.waitForInstance(max_retry=4)
    # check expected_connection_parameter_dict manually due to @classmethod
    instance = self.slap.request(
        ...
        state='started')
    connection_parameter_dict = json.loads(
      instance.getConnectionParameterDict()['_'])
    # tested elsewhere
    connection_parameter_dict.pop('monitor-setup-url', None)
    # comes from instance-monitor.cfg.jinja2, not needed here
    connection_parameter_dict.pop('server_log_url', None)
...
class TestEdgeRegionAdded(TestEdgeRegion):
  ...
  def test(self):
    ...
    # Region was added
    self.expected_connection_parameter_dict[
      'active-region-list'].insert('Region Four')
    self.assertEqual(
      self.expected_connection_parameter_dict,
      connection_parameter_dict
    )
