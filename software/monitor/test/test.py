...
class TestEdgeRegionDestroyed(TestEdgeRegion):
  # Note: This test has to trick out using @classmethods, so it has impact on
  #       MonitorTestMixin, but as test_monitor_setup will be called after
  #       test, there is only assertion for case with destroyed node
  def setUpMonitorConfigurationList(self):
    self.monitor_configuration_list = [
     {
       'htmlUrl': 'https://[%s]:9700/public/feed' % (self._ipv6_address,),
  ...
  def test(self):
    ...
    # hacking with __class__ as those are @classmethods
    self.__class__.instance_parameter_dict[
      'region-dict']['Region Three'].update({'state': 'destroyed'})
...
class TestEdgeRegionAdded(TestEdgeRegion):
  ...
  def test(self):
    ...
    # XXX: few more times, as there is "Unknown Instance" of just added node
    #      which is not caught by any promise, but in the end it shall get
    #      better
