...
Despite external-disk-number, external-disk-size and external-disk-format are
supported fully, it's advised to migrate to external-disk parameter as soon
as possible, as slapos.core `slapos.cfg` `instance_storage_home` can become
obsoleted and removed in future versions.
...
There will be two entries like, whereas order is important::
...
The administrator shall now release the images from automatic detection by
removing files:

 * `etc/.data-disk-amount`
 * `etc/.data-disk-ids`

from the partition (typically `/srv/slappartNN/` directory), otherwise kvm
will fail with::
...
Those files can be used with the new `external-disk`::
