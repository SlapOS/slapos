...
kvm_argument_list = [
  qemu_path, '-enable-kvm', '-name', vm_name, '-vga', 'std', '-smp',
  str(parameter_dict['smp-count']), '-m', '%sM' % (parameter_dict['ram-size'],),
  '-vnc', 'unix:%s,websocket=unix:%s' % (vnc_socket_path, vnc_socket_path),
  '-boot', 'order=cd,menu=on', '-qmp',
  'unix:%s,server,nowait' % qmp_socket_path, '-pidfile', pid_file_path,
  '-msg', 'timestamp=on', '-D', logfile, '-nodefaults',
