...
# Initially the test was pinned to 1.0.462 in order to assure upgrade from
# qmpbackup 0.51, but this tag was not working. Instead of spending time
# debugging it 1.0.498 was selected, with the same component version.
old_software_release_url = 'https://lab.nexedi.com/nexedi/slapos/-/raw/' \
  '1.0.498/software/kvm/software.cfg'
...
if has_kvm:
  ...
  has_tap = bool(kvm_testing.getTopLevelTap(
    SlapOSInstanceTestCase.slap.instance_directory).get('name'))

  if has_tap:
    # the software releases are only worth building when the tests do run
    def setUpModule():
      installSoftwareUrlList(
  ...
  has_tap = False
  # without kvm both software releases would be built just to skip everything
  SlapOSInstanceTestCase = unittest.TestCase
...
skipUnlessTap = unittest.skipUnless(
  has_tap,
  'the tests do not run in a partition having a tap interface, which the'
  ' virtual machines of this suite need to start')


class KvmUpgradeTestCase(
  kvm_testing.KvmPartitionMixin, SlapOSInstanceTestCase):
  ...
  def assertNoVncIsAccessible(self, prefix=''):
    result = requests.get(
      self.getAuthenticatedUrl(
        self.getConnectionParameterDictJson(), prefix=prefix),
      verify=False)
...
class KvmUpgradedMixin:
  ...
  def test_all_partitions_use_new_software_release(self):
    ...
    for computer_partition in self.slap.computer.getComputerPartitionList():
      try:
        software_release_dict[computer_partition.getId()] = \
        ...
        continue
    # a software release requested by path is served over HTTP by the
    # slapos-sr-testing instance, and the partitions record that URL instead
    self.assertNotIn(old_software_release_url, software_release_dict.values())
    self.assertEqual(
      set(software_release_dict.values()),
      {software_release_dict[self.computer_partition.getId()]},
      'partitions on different software releases: %s'
      % (software_release_dict,))
...
class TestKvmResilientUpgrade(KvmUpgradedMixin, KvmUpgradeTestCase):
  ...
  instance_parameter_dict = {'resiliency-backup-periodicity': '0 0 1 1 *'}
  ...
  def setUpOldInstance(cls):
    super().setUpOldInstance()
    # back up twice with the old software release, so that the upgrade finds a
    # complete backup chain of the old style to migrate, increment included
    cls.backup()
    cls.backup()
  ...
  def test_old_backup_chain_was_complete(self):
    ...
    self.assertEqual(
      1,
      len([q for q in self.old_chain_file_list if q.startswith('INC-')]),
      self.old_chain_file_list)
  ...
  def test_backup_chain_is_migrated(self):
    ...
    self.assertIn(
      ...
      'bitmaps', status_text)
    # the migration drops the chain whatever its size, so there is never
    # anything to roll a full backup for right after it
    self.assertNotIn('Rolling full backup', status_text)
