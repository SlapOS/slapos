...
class ExternalDiskModernMixin(object):
  ...
  def prepareEnv(self):
    ...
    self.second_disk = os.path.join(self.kvm_instance_partition, self.second_disk_name)
    subprocess.check_call([
...
@skipUnlessKvm
class TestExternalDiskModern(InstanceTestCase, ExternalDiskModernMixin, ExternalDiskMixin):
  def test(self):
    self.prepareEnv()
    self.waitForInstance()
...
@skipUnlessKvm
class TestExternalDiskModernConflictAssurance(InstanceTestCase, ExternalDiskModernMixin, ExternalDiskMixin):
  def test(self):
    self.prepareEnv()
