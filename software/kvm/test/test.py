...
class TestBootImageUrlList(InstanceTestCase):
  ...
  def startImageHttpServer(self):
    ...
    self.fake_image_md5sum = hashlib.md5sum(fake_image_content).hexdigest()
    with open(
    ...
    self.fake_image2_md5sum = hashlib.md5sum(fake_image2_content).hexdigest()
    with open(
      os.path.join(self.image_directory, self.fake_image_md5sum), 'wb') as fh:
    ...
    self.fake_image = '/'.join(server.server_address, self.fake_image_md5sum)
    self.fake_image_2 = '/'.join(
      server.server_address, self.fake_image2_md5sum)
