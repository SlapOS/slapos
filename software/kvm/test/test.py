...
class KvmMixin:
  ...
  @classmethod
  def getAuthenticatedUrl(cls, connection_parameter_dict, url_param='url'):
    parsed_url = urlparse(connection_parameter_dict[url_param])
    return parsed_url._replace(
    netloc='{}:{}@[{}]:{}'.format(
      connection_parameter_dict['username'],
      connection_parameter_dict['password'],
      parsed_url.hostname,
      parsed_url.port,
    )).geturl()
...
class TestAccessDefaultAdditional(MonitorAccessMixin, KVMTestCase):
  ...
  def test(self):
    ...
    authenticated_url = self.getAuthenticatedUrl(connection_parameter_dict, 'url-additional')
    result = requests.get(authenticated_url, verify=False)
    self.assertEqual(
      httplib.OK,
      result.status_code
...
class TestAccessKvmCluster(MonitorAccessMixin, KVMTestCase):
  ...
  def test(self):
    connection_parameter_dict = self.getConnectionParameterDictJson()
    authenticated_url = self.getAuthenticatedUrl(connection_parameter_dict, 'KVM0-url')
    result = requests.get(authenticated_url, verify=False)
    self.assertEqual(
      httplib.OK,
      result.status_code
...
class TestAccessKvmClusterAdditional(MonitorAccessMixin, KVMTestCase):
  ...
  def test(self):
    ...
    authenticated_url = self.getAuthenticatedUrl(connection_parameter_dict, 'KVM0-url-additional')
    result = requests.get(authenticated_url, verify=False)
    self.assertEqual(
      httplib.OK,
      result.status_code
...
class TestAccessKvmClusterBootstrap(MonitorAccessMixin, KVMTestCase):
  ...
  def test(self):
    connection_parameter_dict = self.getConnectionParameterDictJson()
    authenticated_url = self.getAuthenticatedUrl(connection_parameter_dict, 'test-machine1-url')
    result = requests.get(authenticated_url, verify=False)
    self.assertEqual(
      httplib.OK,
      result.status_code
    ...
    authenticated_url = self.getAuthenticatedUrl(connection_parameter_dict, 'test-machine2-url')
    result = requests.get(authenticated_url, verify=False)
    self.assertEqual(
      httplib.OK,
      result.status_code
