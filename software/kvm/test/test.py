...
class TestInstanceResilientBackupMixin(CronMixin, KvmMixin):
  ...
  def assertExporterStatus(
    ...
    post_take_over_upgrade_recover=False,
    migrated_old=False,
    ):
    ...
    post_take_over_upgrade_recover_text = \
      'Post take-over or post qmpbackup upgrade cleanup'
    if post_take_over_upgrade_recover:
      self.assertIn(post_take_over_upgrade_recover_text, status_text)
    else:
      self.assertNotIn(post_take_over_upgrade_recover_text, status_text)
...
class TestInstanceResilientBackupExporterOldStyleMigration(
  ...
  def test(self):
    ...
    self.assertEqual(
      len(glob.glob(self.getKvmExportPartitionBackupPath('INC-*.qcow2'))),
      0)
    self.assertExporterStatus(status_text, migrated_old)
