...
class TestParameterDefault(InstanceTestCase, KvmMixin):
  ...
  def test_cpu_count_default(self):
    self._test({}, "init_smp_count = 4")

  def test_cpu_count_default_max(self):
    self._test({}, "smp_max_count = 5")

  def test_cpu_count_set(self):
    self._test({'cpu-count': 2}, "init_smp_count = 2")

  def test_cpu_count_set_max(self):
    self._test({'cpu-count': 2}, "smp_max_count = 3")

  def test_ram_size_default(self):
    self._test({}, "init_ram_size = 16384")

  def test_cpu_count_default_max(self):
    self._test({}, "ram_max_size = '16896'")
...
class TestParameterCluster(TestParameterDefault):
  ...
  def mangleParameterDiict(self, parameter_dict):
    return {'_': json.dumps({
      "kvm-partition-dict": {
        "KVM0": { parameter_dict }
      }
