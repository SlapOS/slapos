...
class CustomInboundCertificateTestCase(SlapOSInstanceTestCase):
  ...
  def getRelayCertificatePathDict(self, relay):
    return {
      ...
      "custom-cert": self.partitionPath(relay, 'etc', 'postfix', 'ssl_inbound', 'custom.crt'),
      "custom-key": self.partitionPath(relay, 'etc', 'postfix', 'ssl_inbound', 'custom.key'),
    }
  ...
  def assertCustomCertificateWritten(self, path_dict, certificate):
    self.assertEqual(
      ...
      self.readFile(path_dict["custom-cert"]).strip(),
    )
    self.assertEqual(
      certificate["key"].encode().strip(),
      self.readFile(path_dict["custom-key"]).strip(),
    )
  ...
  def test_custom_inbound_certificate_lifecycle(self):
    ...
    self.assertServedInboundCertificate(valid_certificate["cert"])
