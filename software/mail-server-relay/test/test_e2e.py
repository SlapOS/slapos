...
def setUpModule():
  ...
  installSoftwareUrlList(SlapOSInstanceTestCase, [RELAY_SR, EMAIL_SR], debug)


class E2E(SlapOSInstanceTestCase):
  instance_max_retry = 4
  domain_list = ["mail%d.domain.lan" % i for i in range(1, 4)]
  direct_relay_domain = "mail%d.domain.lan" % (len(domain_list) + 1)
    ...
    parameters = serialize({
     "default-proxy-config": proxy_config,
      "outbound-domain-whitelist": [
        "mail%d.domain.lan" % i for i in range(1, 5)
      ],
      "topology": {
          "relay-foo": {
            "state": "started"
  ...
  def requestMailServer(cls, domain, state, extra=None):
    ...
    mail_server.testmail = 'testmail@' + domain
    return mail_server
  ...
  @classmethod
  def requestRelayShared(cls, parameters, state):
    return cls.slap.request(
      software_release=RELAY_SR,
      partition_reference=parameters['name'],
  ...
  def requestDefaultInstance(cls, state="started"):
    ...
    cls.mail_servers = [
      cls.requestMailServer(domain, state) for domain in cls.domain_list
    ]
    cls.direct_relay_shared = cls.requestRelayShared({
      "name": cls.direct_relay_domain,
      "mail-server-host": "::1",
  ...
  def send_email(self, mail_server, mail_recipient, body, send_as=None):
    smtp_params = self.getConnectionDict(mail_server)
  ...
  def check_inbox(self, mail_server, expected):
    imap_params = self.getConnectionDict(mail_server)
    host, port = imap_params['imap-smtp-ipv6'], imap_params['imap-port']
    def check_email():
      with imaplib.IMAP4(host, port, timeout=10) as imap:
        imap.login(mail_server.testmail, self.testmail_password)
        imap.select("INBOX")
    ...
    try_until(
      ...
      timeout=60,
      interval=2,
      err_msg=f"Email with '{expected}' not received by {mail_server.testmail}"
    )

  def check_mail_e2e(self, mail_server, mail_recipient, body, send_as=None):
    self.send_email(mail_server, mail_recipient, body, send_as)
  ...
  def getRelayHost(self):
    ...
    cluster_params = self.getConnectionDict(self.relay_cluster)
    ...
    for line in dns_entries.strip().split('\n'):
      if 'MX' in line:
        ...
        if len(parts) >= 4:
          ...
          return relay_host
    self.fail(f"Could not find relay host in DNS entries: {dns_entries}")
  ...
  def test_send_email_from_external_via_relay(self):
    ...
    relay_host = self.getRelayHost()
    msg = "Subject: Test Email from External\n\nThis is a test email from external via relay."
    with smtplib.SMTP(relay_host, self.relay_inbound_port, timeout=10) as smtp:
    ...
    self.check_inbox(mail1, "This is a test email from external via relay.")

  def check_not_in_inbox(self, mail_server, unexpected_content, wait_time=30):
      ...
      for email_id in data[0].split():
        ...
        if result == 'OK':
          ...
          self.assertNotIn(
            ...
            f"Email with unexpected content '{unexpected_content}' was found in {mail_server.testmail}'s inbox"
          )
  ...
  @classmethod
  def getDirectRelayConnection(cls):
    params = cls.getConnectionDict(cls.direct_relay_shared)
    return params['outbound-user'], params['outbound-password']

  def test_relay_shared_output(self):
    params = self.getConnectionDict(self.direct_relay_shared)
    self.assertTrue(
  ...
  def test_relay_password_auth_legitimate(self):
    ...
    mail1 = self.mail_servers[0]
    relay_host = self.getRelayHost()
    user, password = self.getDirectRelayConnection()
    msg = "Subject: Password Auth Legit\n\nPassword auth legitimate test."
    with smtplib.SMTP(relay_host, self.relay_outbound_port, timeout=10) as smtp:
      smtp.starttls()
      smtp.login(user, password)
      smtp.sendmail(
        from_addr='example@' + self.direct_relay_domain,
        to_addrs=[mail1.testmail],
        msg=msg,
      )
    self.check_inbox(mail1, "Password auth legitimate test.")
