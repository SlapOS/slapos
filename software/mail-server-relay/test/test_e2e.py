...
class E2E(SlapOSInstanceTestCase):
  ...
  def setUpClass(cls):
    ...
    cls.direct_relay_shared.login = cls.getDirectRelayLogin()

  @classmethod
  def getDirectRelayLogin(cls):
  ...
  def test_relay_password_auth_legitimate(self):
    ...
    with smtplib.SMTP(*self.relay_outbound_addr, timeout=10) as smtp:
      smtp.starttls()
      smtp.login(*self.direct_relay_shared.login)
      smtp.sendmail(
  ...
  def test_relay_auth_impersonation_blocked(self):
  ...
  def test_relay_auth_rejected_without_tls(self):
