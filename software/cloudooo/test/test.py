...
setUpModule, _CloudOooTestCase = makeModuleSetUpAndTestCaseClass(
    str((Path(__file__).parent / "../software.cfg").absolute())
)


def open_cloudooo_conection(url: str) -> xmlrpclib.ServerProxy:
    """
...
class CloudOooTestCase(_CloudOooTestCase):
    ...
    def setUp(self) -> None:
        ...
        self.server = open_cloudooo_conection(self.url)
...
def script_test_basic(test_case: CloudOooTestCase) -> bytes:
    """
    Tries to execute a hello world script.

    Args:
        test_case: The test case inside which this function is
          called (used to call the conversion routine).
...
def _convert_html_to_text(
    ...
    *,
    ...
    """
    with open_cloudooo_conection(url) as server:
        return convert_file(
            src_html,
            "html",
            "txt",
