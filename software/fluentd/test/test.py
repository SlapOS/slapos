...
class WendelinTutorialTestCaseMixin:
  ...
  def serve(self, bind, port, request_handler_class):
...
<match tag.name>
  ...
  def test_send_data(self):
    tag, data, header = msgpack.unpackb(
      self.serve(self.computer_partition_ipv6_address, FLUENTD_PORT, FluentdHTTPRequestHandler),
      raw=True,
    )
...
class GatewayConfTestCase(WendelinTutorialTestCaseMixin, FluentdDefaultTestCase):
  ...
  def getInstanceParameterDict(cls):
    return {
      ...
      'wendelin-frontend-url': f'http://[{cls.computer_partition_ipv6_address}]:{cls._wendelin_port}/erp5/portal_ingestion_policies/default',
      'username': 'foo',
      'password': 'bar',
      'flush-interval': '1'
    }
  ...
  def test_wendelin_data_forwarding(self):
    ...
    print(self.serve(self.computer_partition_ipv6_address, self._wendelin_port, WendelinHTTPRequestHandler))

    self.assertEqual(
      data,
      msgpack.unpackb(
        self.serve(self.computer_partition_ipv6_address, self._wendelin_port, WendelinHTTPRequestHandler)),
    )
...
class NoTlsDefaultTestCase(FluentdDefaultTestCase):
  ...
  def getInstanceParameterDict(cls):
    return {
      'port': cls._fluentd_port,
      'wendelin-frontend-url': f'http://[{cls.computer_partition_ipv6_address}]:{cls._wendelin_port}',
      'username': 'foo',
      'password': 'bar'
    }
