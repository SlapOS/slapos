...
class OrsWendelinGatewayTutorialTestCase(FluentdTestCase):

  @classmethod
  def getInstanceParameterDict(cls):
    return {
      'port': cls._fluentd_port,
      'fluentd-shared-key': 'secret',
      'ors-wendelin-frontend-url': f'http://[{cls.computer_partition_ipv6_address}]:{cls._ors_wendelin_port}',
      'inituser-login': 'foo',
      'inituser-password': 'bar'
    }
  ...
  def setUpClass(cls):
    ...
    fluentd_port = findFreeTCPPort(cls.computer_partition_ipv6_address)
    cls._fluentd_port = fluentd_port
    ors_wendelin_port = findFreeTCPPort(cls.computer_partition_ipv6_address)
    cls._ors_wendelin_port = ors_wendelin_port
