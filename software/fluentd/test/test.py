...
class FluentdHTTPRequestHandler(StreamRequestHandler):
  ...
  def handle(self):
    ...
    if len(self.data) > 0:
      FluentdHTTPRequestHandler.received_data = self.data
...
print("%s")''' % "\t".join(measurementList)
  ...
  def test_send_data(self):
    unpacked_msg = msgpack.unpackb(
      self.serve(FLUENTD_PORT, FluentdHTTPRequestHandler),
      raw=True
    )
    self.assertEqual(b'tag.name', unpacked_msg[0])
    self.assertEqual(self.measureDict(), msgpack.unpackb(unpacked_msg[1])[-1])
    self.assertEqual({b'compressed': b'text', b'size': 1}, unpacked_msg[-1])
...
<match tag.name>
  ...
  def test_wendelin_data_forwarding(self):
    ...
    sock.sendall(
      b'\x93\xa8tag.name\xdb\x00\x00\x00%s\x82\xa4size\x01\xaacompressed\xa4text'
      % chr(len(packed_data)).encode() + packed_data
    )
    sock.close()
