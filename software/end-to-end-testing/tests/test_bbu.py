...
class WebsocketTestClass(e2e.EndToEndTestCase):
    ...
    def setUpClass(cls):
        try:
            ...
            cls.enb_instance_name = 'e2e-cb003-enb'
            cls.enb_cell_instance_name = 'e2e-cb003-enb.CELL1'
            cls.enb_ru_instance_name = 'e2e-cb003-enb.RU1'
            cls.core_network_instance_name = 'e2e-cb003-mme'
            cls.core_network_sim_instance_name = 'e2e-cb003-mme/sim1'
            ...
            cls.parameters['enb'] = {
                  ...
                  'amarisoft_version': '2022-12-16.1739548131'
            }
            cls.parameters['enb#cell'] = {
                'cell_type':    'lte',
                'cell_kind':    'enb',
                'rf_mode':      'fdd',
                'bandwidth':    20,
                ...
                'ru':           {
                    'ru_type':  'ru_ref',
                    'ru_ref':   cls.enb_ru_instance_name
                },
                  'amarisoft_version': '2022-12-16.1739548131'
            }
            cls.parameters['enb#ru'] = {
                  'ru_type':      'lopcomm',
                  'ru_link_type': 'cpri',
                  'cpri_link':    {
                      'sdr_dev':  0,
    ...
    def update_service(cls, name, state, parameters=None, lock=None):
        ...
        if parameters:
            ...
            # Check if '_' key exists in parameter_dict
            if '_' in instance_infos.parameter_dict:
                parameters = {'_': json.dumps(instance_infos.parameter_dict['_'])}
            else:
                cls.logger.warning(f"Key '_' not found in parameter_dict for {instance_name}. Using empty parameters.")
                parameters = {'_': json.dumps({})}  # Use empty parameters or handle as needed
        ...
        while lock and not shared:
            ...
            lock = time.time()
            # Safely access 'lock' key with a default value
