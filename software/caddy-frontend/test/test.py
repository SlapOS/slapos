...
class TestDataMixin(object):
  ...
  def test_supervisor_state(self):
    ...
    data_replacement_dict = {
      'generic': '{hash-%s}' % (generateHashFromFiles(hash_file_list),)
    }
    ...
    for backend_haproxy_wrapper_path in glob.glob(os.path.join(
      ...
      data_replacement_dict[
        'backend-haproxy-%s' % (partition_id)] = '{hash-%s}' % (
  ...
  def test_cluster_request_instance_parameter_dict(self):
    ...
    for slave_reference, slave_value in\
          self.getSlaveConnectionParameterDictDict().items():
      data_replacement_dict[
        '@@%s_key-generate-auth-url@@' % slave_reference] = slave_value[
        'key-generate-auth-url'].split('/')[-2]
      data_replacement_dict[
        '@@%s_key-upload-url@@' % slave_reference] = slave_value[
