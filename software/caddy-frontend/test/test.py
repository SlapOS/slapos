...
class TestMasterRequest(HttpFrontendTestCase, TestDataMixin):
  ...
  def test(self):
    self.runComputerPartition(max_quantity=5)
    ...
    self.assertEqual(
      {
        ...
        'domain': 'None',
        'kedifa-caucase-url': 'http://[%s]:%s' % (GLOBAL_IPV6, CAUCASE_PORT),
...
class SlaveHttpFrontendTestCase(HttpFrontendTestCase):
  ...
  def setUpMaster(cls):
    ...
    # properlly
    cls.runComputerPartition(max_quantity=6)
  ...
  def setUpClass(cls):
    try:
      ...
      try:
        ...
        cls.setup_master_excption = exc
      try:
        cls.setUpSlaves()
      except Exception as exc:
        cls.setup_slave_exception = exc
    except Exception:
      cls.tearDownClass()
      raise

  def test_setUpMasterState(self):
    self.assertEqual(None, self.setup_master_exception)

  def test_setUpSlaveState(self):
    self.assertEqual(None, self.setup_slave_exception)
...
class AutoRestartTestCase(SlaveHttpFrontendTestCase):
  ...
  def test_hashes(self):
    ...
    expected_process_names = [
      'kedifa-{hash}-on-watch',
      'caucased-{hash}-on-watch',
      'caucase-updater-{hash}-on-watch',
      'expose-csr_id-{hash}-on-watch',
...
class TestSlaveSlapOSMasterCertificateCompatibility(
  ...
  def test_ssl_from_slave(self):
    ...
    self.assertEqual(
      self.ssl_from_slave_ca_certificate_pem,
      der2pem(result.peercert))
