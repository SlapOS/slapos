...
* **Changelog** — the ``CHANGES.rst`` update is complete and correct:
  ...
  * *Right audience* — operator ↔ ``instance-input-schema.json`` / ``instance-output-schema.json``; user ↔ ``instance-slave-input-schema.json`` / ``instance-slave-output-schema.json`` or served-traffic behaviour. Judge by impact, not only by the files touched — a real effect may be missing its schema change.
  ...
  * *Unreleased* — the entry sits under ``Unreleased`` with no version number (assigned only at release).
