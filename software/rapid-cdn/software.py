...
def _prune_stale_shared_overrides(
    error_pages_dir, active_references, retention_delay, now, state_file):
  """Remove override files of slaves absent longer than retention_delay.

  A slave missing from active_references is kept (first-absent time tracked in
  state_file) until retention_delay seconds pass; 0 removes immediately.
  """
  import json
  import os
  import shutil
  ...
  shared_haproxy_dir = os.path.join(error_pages_dir, 'haproxy', 'shared')
  refs_on_disk = set(os.listdir(shared_source_dir)) \
    if os.path.isdir(shared_source_dir) else set()

  try:
    with open(state_file) as f:
      absent_since = json.load(f)
  except Exception:
    absent_since = {}

  pruned = set()
  for ref in refs_on_disk:
    if ref in active_references:
      absent_since.pop(ref, None)
      continue
    if now - absent_since.setdefault(ref, now) >= retention_delay:
      shutil.rmtree(os.path.join(shared_source_dir, ref), ignore_errors=True)
      shutil.rmtree(os.path.join(shared_haproxy_dir, ref), ignore_errors=True)
      absent_since.pop(ref, None)
      pruned.add(ref)
  # Forget slaves whose source directory is already gone.
  for ref in [r for r in absent_since if r not in refs_on_disk]:
    absent_since.pop(ref, None)
...
def error_page_manager_main():
  ...
  # A stopped slave is seen as destroyed by the master, so keep its override
  # for instance-retention-delay before removing it.
  RETENTION_DELAY = int(config.get('retention_delay', 7776000))
  ABSENT_STATE_FILE = config.get(
    'absent_state_file',
    os.path.join(ERROR_PAGES_DIR, 'shared-absent-since.json'))
...
<body>
  ...
  with _lock:
    for code in SUPPORTED_CODES:
      _publish_haproxy_file('cluster', code)
    # Prune slaves gone past retention, then re-publish surviving overrides.
    _prune_stale_shared_overrides(
      ERROR_PAGES_DIR, set(SHARED_TOKEN_MAP.values()),
      RETENTION_DELAY, time.time(), ABSENT_STATE_FILE)
    shared_source_dir = os.path.join(ERROR_PAGES_DIR, 'shared')
