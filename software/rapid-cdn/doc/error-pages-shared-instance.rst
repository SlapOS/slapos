...
``#`` are silently dropped by the EPM.  This syntax is **reserved**
for a possible future feature for declaring custom response headers;
no header support is implemented in this version.
* Response: ``204 No Content`` on success.
* The change takes effect immediately — no action needed on your side.

Multilingual pages are supported by writing a single HTML document
that bundles all languages and uses a tiny piece of inline JavaScript
to pick the right one based on the visitor's browser locale.  The
built-in default pages use this pattern.  See the section
*Multi-language pages with client-side switching* in the operator
guide for a complete worked example you can adapt.
