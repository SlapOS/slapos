...
backend _health-check-default
  ...
  def test_health_check_failover_url_https_only_false(self):
    ...
    # is `<backend>failover-url?a=b&c=`, so c5's set-path/set-query split
    # makes the eventual upstream request hit `<backend>failover-url<path>
    # ?a=b&c=` — register the 503 response under that exact URL.
    for path in ['/failoverpath', '/' + cached_path]:
      result = mimikra.config(
        self.backend_url + 'failover-url' + path + '?a=b&c=',
        headers=d2h({
