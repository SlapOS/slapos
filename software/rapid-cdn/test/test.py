...
class HttpFrontendTestCase(SlapOSInstanceTestCase):
  ...
  def assertResponseHeaders(
    ...
    content_length=True, cache_hit=False):
    ...
    if via:
      ...
      if via_frontend_only:
        ...
        # ATS records the upstream-connection scheme in its Via entry. On a
        # cache miss/populate (and on revalidation), upstream is the backend
        # over HTTPS → cache hop reports "https/1.0". On a pure cache hit
        # there is no upstream connection and ATS falls back to "http/1.0".
        cache_hop_proto = 'http/1.0' if cache_hit else 'https/1.0'
        self.assertEqual(
          expected_via + 'HTTP/1.1 rapid-cdn-backend-%(via_id)s, '
          '%(cache_hop_proto)s rapid-cdn-cache-%(via_id)s '
          'HTTP/%(client_version)s rapid-cdn-frontend-%(via_id)s' % dict(
            via_id=via_id, client_version=client_version,
            cache_hop_proto=cache_hop_proto),
          via_header
        )
      else:
        self.assertEqual(
...
class TestSlave(SlaveHttpFrontendTestCase, TestDataMixin, AtsMixin):
  ...
  retries 3""", content)
    ...
    if self.max_client_version != '3.0':
      # curl behaviour for HTTP/3 is different than for HTTP/2.0 and below
  ...
  def test_backend_haproxy_bare_slash_url_with_query_keeps_leading_slash(self):
    section = self._get_backend_haproxy_backend_section('_Url')
    self.assertIn(
      "http-request set-path /?a=b&c=%[path,regsub('^/$','')]",
      section)

  def test_backend_haproxy_url_path_with_query_preserves_both(self):
    section = self._get_backend_haproxy_backend_section(
      '_url-path-with-query')
    self.assertIn(
      "http-request set-path /path?a=b%[path,regsub('^/$','')]",
      section)
  ...
  def test_auth_to_backend(self):
    ...
    try:
      ...
      self.assertIn(cm.exception.command_returncode, [55, 56])
...
<note>
      ...
      if verb == 'CONNECT':
        ...
        if verb != 'HEAD':
          try:
            ...
            raise ValueError('JSON decode problem in:\n%s' % (result.text,))
          self.assertEqual('/' + verb, j['Path'], verb)
          self.assertEqual(verb, j['Verb'], verb)
  ...
  def test_enable_cache_ims_request(self):
    ...
    result_headers = self.assertResponseHeaders(
      check_result, cached=True, age=True)
  ...
  def test_enable_cache_last_modified(self):
    ...
    self.assertEqual(check_result.status_code, http.client.OK)
    self.assertResponseHeaders(
      check_result, cached=True, age=True, cache_hit=True)
    self.assertEqual(
      check_result.headers['Last-Modified'], original_last_modified)
    self.assertEqual(check_result.text, data)
...
class TestSlaveHealthCheck(SlaveHttpFrontendTestCase, TestDataMixin, AtsMixin):
  ...
  def setUpAssertionDict(cls):
    ...
    cls.assertion_dict = {
      'health-check-disabled': """\
backend _health-check-disabled
  timeout server 12s
  timeout connect 5s
  retries 3
  server _health-check-disabled-backend %s""" % (backend,),
      'health-check-connect': """\
backend _health-check-connect
  timeout server 12s
  timeout connect 5s
  retries 3
...
backend _health-check-default
  ...
  def test_health_check_failover_url_https_only_false(self):
    ...
    # in order to check ATS behaviour. The slave's health-check-failover-url
    # is `<backend>failover-url?a=b&c=`, and the rendered haproxy combines
    # path?query into a single set-path, so the eventual upstream request
    # hits `<backend>failover-url?a=b&c=<path>` — register the 503 response
    # under that exact URL.
    for path in ['/failoverpath', '/' + cached_path]:
      result = mimikra.config(
        self.backend_url + 'failover-url?a=b&c=' + path,
        headers=d2h({
  ...
  def test_backend_haproxy_bare_slash_failover_url_emits_no_rewrite(self):
    section = self._get_backend_haproxy_backend_section(
      '_health-check-failover-url-ssl-proxy-verified-failover')
    self.assertNotIn('http-request set-path', section)

  def test_backend_haproxy_bare_slash_failover_url_with_query_keeps_leading_slash(
      self):
    section = self._get_backend_haproxy_backend_section(
      '_health-check-failover-url-bare-slash-with-query-failover')
    self.assertIn(
      "http-request set-path /?a=b&c=%[path,regsub('^/$','')]",
      section)
