...
def fakeHTTPSResult(domain, path, port=HTTPS_PORT,
  ...
  if headers is None:
    ...
    # simplification for cases when headers are passed as dict
...
class TestHandler(BaseHTTPRequestHandler):
  ...
  def do_GET(self):
    def doJsonReponse():
      ...
      response = json.dumps(response, indent=2).encode()
    ...
    if config is None:
      self.protocol_version = self.DEFAULT_CONFIGURATION['Protocol-Version']
      self.send_response(int(self.DEFAULT_CONFIGURATION['Status-Code']))
      doJsonReponse()
      return
    ...
    if len(config['configuration']['Body']) == 0 \
       and 'content-type' not in config['headers'] \
       and 'content-length' not in config['headers']:
       doJsonReponse()
    else:
      if self.DEFAULT_HEADERS is not None:
        for header_name, header_value in self.DEFAULT_HEADERS.items():
...
class HttpFrontendTestCase(SlapOSInstanceTestCase):
  ...
  def startServerProcess(cls):
    ...
    default_headers = http.client.HTTPMessage()
    default_headers.add_header('X-Backend-Identification', 'netloc')
  ...
  def startAuthenticatedServerProcess(self):
    ...
    default_headers = http.client.HTTPMessage()
    default_headers.add_header('X-Backend-Identification', 'Auth Backend')
    default_headers.add_header(
...
class TestSlave(SlaveHttpFrontendTestCase, TestDataMixin, AtsMixin):
  ...
  @classmethod
  def setUpClass(cls):
    super(TestSlave, cls).setUpClass()
    headers = http.client.HTTPMessage()
    headers.add_header(
      'Server', 'TestBackend'
    )
    ...
    headers.add_header(
      'Via', 'http/1.1 backendvia'
    )
    config_result = mimikra.config(
      cls.backend_url + 'test-path/deeper',
      headers=headers
    )
