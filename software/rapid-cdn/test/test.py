...
class TestHandler(BaseHTTPRequestHandler):
  ...
  def do_CONFIG(self):
    ...
    self.wfile.write(json.dumps(reply, indent=2, cls=ConfigurationReplyEncoder).encode())
  ...
  def assertResponseHeaders(
    ...
    if 'Connection' in result.headers:
      if assertSingleHeader().lower() == 'keep-alive':
        pop_header_list.append('Connection')
    ...
    if via:
      pop_header_list.append('Via')
      via = result.headers.get_all('Via')
      if cached:
        self.assertEqual(
          'http/1.1 backendvia, '
...
<note>
  ...
  def test_enable_cache_ims_request(self):
    ...
    headers = setUpHeaders([
      ...
      ('X-Config-Reply-Header-Cache-Control', 'max-age=10, stale-if-error=604800, stale-while-revalidate=15, public'),
