...
class TestErrorPageManagerNewSlave(SlapOSInstanceTestCase):
  ...
  def test_new_slave_token_accepted_after_instance_update(self):
    """Slave added after EPM startup can upload error pages (204, not 401)."""
    # Add slave-B to the instance and re-run buildout
    self.__class__._instance_parameter_dict = {
      '_': json.dumps({
        'monitor-password': 'test-monitor-password',
        'monitor-httpd-port': self._EPM_MONITOR_PORT,
        'slave-list': [
          {'slave_reference': self.SLAVE_A_REF},
          {'slave_reference': self.SLAVE_B_REF},
        ],
      }),
    }
    self.requestDefaultInstance()
    ...
    # Read updated connection parameters to get slave-B's upload URL
    conn = json.loads(
      self.requestDefaultInstance().getConnectionParameterDict()['_'])
    slave_info = json.loads(conn['slave-error-page-information'])
    self.assertIn(self.SLAVE_B_REF, slave_info,
                  'slave-B should appear in slave-error-page-information')

    upload_url = slave_info[self.SLAVE_B_REF]['upload-url'] + '503'
    result = mimikra.put(upload_url,
