...
class TestErrorPageManager(SlapOSInstanceTestCase):
  ...
  def test_default_pages_fit_haproxy_buffer(self):
    """Built-in default .http files must fit in HAProxy's default tune.bufsize.

    HAProxy reads the entire errorfile into a single buffer (default 16 KiB).
    If a default page grows beyond that HAProxy refuses to load it.
    """
    HAPROXY_DEFAULT_BUFSIZE = 16384
    for code in self.SUPPORTED_CODES:
      result = self._get('%s/cluster/%s.http' % (self.base_url, code))
      self.assertEqual(result.status_code, 200)
      size = len(result.text.encode('utf-8'))
      self.assertLess(
        size, HAPROXY_DEFAULT_BUFSIZE,
        'default page for %s is %d bytes, exceeds HAProxy buffer (%d)' % (
          code, size, HAPROXY_DEFAULT_BUFSIZE))
