...
class TestErrorPageManager(SlapOSInstanceTestCase):
  ...
  def test_operator_web_ui_save_one_preserves_other_edits(self):
    """Operator: saving one code must not discard edits typed into the others."""
    codes = ('500', '502', '503')
    for c in codes:
      self._delete(self.operator_url + c)
    try:
      resp = self._post_form(self.operator_url, {
        'action': 'save_500',
        'html_500': 'SAVED-500-MARKER',
        'html_502': 'UNSAVED-502-MARKER',
        'html_503': 'UNSAVED-503-MARKER',
      })
      self.assertEqual(resp.status_code, 200)
      # the other codes' edits survive in the re-rendered form
      self.assertIn('UNSAVED-502-MARKER', resp.text)
      self.assertIn('UNSAVED-503-MARKER', resp.text)
      # only 500 was persisted; the merely-edited 502/503 were not
      self.assertIn('SAVED-500-MARKER', self._get(self.operator_url + '500').text)
      for c in ('502', '503'):
        self.assertEqual(self._get(self.operator_url + c).text, '')
    finally:
      for c in codes:
        self._delete(self.operator_url + c)
...
class TestBigFileCache(SlaveHttpFrontendTestCase, AtsMixin):
  ...
  def _streamPaced(cls, conn, path, size, declared=None):
    # `declared` is what the header announced when the origin means to write
    # less than that, so the log can record a truncation rather than an end.
  ...
  def _bigHandler(cls, conn):
    # Honours a byte range, so the range cases are not accidentally answered
    # with a plain 200.
  ...
  def _stallHandler(cls, conn):
    # Never closes, so backend-haproxy's request-timeout is the only thing
    # that can end the response, and the announced length is what makes the
    # result detectably short.
  ...
  def _fetch(self, domain, path, abort_at=None, range_spec=None, timeout=120):
    # Closing at `abort_at` is what a client walking away does: the frontend
    # haproxy sees the close and drops its own connection to trafficserver.
    request = (
