...
class TestErrorPageRetention(unittest.TestCase):
  """Unit tests for software._prune_stale_shared_overrides."""

  @classmethod
  def setUpClass(cls):
    import importlib.util
    spec = importlib.util.spec_from_file_location(
      '_software_src_retention',
      os.path.normpath(os.path.join(
        os.path.dirname(__file__), '..', 'software.py')))
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    cls._prune = staticmethod(mod._prune_stale_shared_overrides)

  def _make_override(self, error_pages_dir, ref, code='503'):
    src = os.path.join(error_pages_dir, 'shared', ref)
    hap = os.path.join(error_pages_dir, 'haproxy', 'shared', ref)
    os.makedirs(src)
    os.makedirs(hap)
    open(os.path.join(src, code + '.html'), 'w').close()
    open(os.path.join(hap, code + '.http'), 'w').close()

  def _exists(self, error_pages_dir, ref):
    return os.path.isdir(os.path.join(error_pages_dir, 'shared', ref))

  def test_active_slave_is_kept(self):
    tmp = tempfile.mkdtemp()
    try:
      self._make_override(tmp, 'a')
      state = os.path.join(tmp, 'absent.json')
      self.assertEqual(self._prune(tmp, {'a'}, 100, 1000.0, state), set())
      self.assertTrue(self._exists(tmp, 'a'))
    finally:
      shutil.rmtree(tmp)

  def test_absent_slave_kept_within_retention_then_pruned(self):
    tmp = tempfile.mkdtemp()
    try:
      self._make_override(tmp, 'a')
      state = os.path.join(tmp, 'absent.json')
      # first sight of absence starts the clock
      self.assertEqual(self._prune(tmp, set(), 100, 1000.0, state), set())
      self.assertTrue(self._exists(tmp, 'a'))
      self.assertEqual(self._prune(tmp, set(), 100, 1050.0, state), set())
      self.assertTrue(self._exists(tmp, 'a'))
      self.assertEqual(self._prune(tmp, set(), 100, 1100.0, state), {'a'})
      self.assertFalse(self._exists(tmp, 'a'))
    finally:
      shutil.rmtree(tmp)

  def test_zero_retention_prunes_immediately(self):
    tmp = tempfile.mkdtemp()
    try:
      self._make_override(tmp, 'a')
      state = os.path.join(tmp, 'absent.json')
      self.assertEqual(self._prune(tmp, set(), 0, 1000.0, state), {'a'})
      self.assertFalse(self._exists(tmp, 'a'))
    finally:
      shutil.rmtree(tmp)
