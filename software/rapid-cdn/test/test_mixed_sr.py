...
* SERVES (1.0.496) -- an OLD frontend recent enough to share the current
  ...
  certificate from the NEW kedifa), and the slave propagates to both frontends.
...
class _MixedFrontendServesTestMixin(_MixedFrontendSoftwareReleaseMixin):
  ...
  def test_mixed_software_release(self):
    ...
    # is compatible across the two SR versions.
...
class TestMixedFrontendSoftwareRelease1_0_496(
  ...
  old_software_release_url = SERVES_SOFTWARE_RELEASE_URL
