...
class _MixedFrontendUpgradeTestMixin(_MixedFrontendSoftwareReleaseMixin):
  ...
  def test_upgrade_old_frontend_to_new(self):
    ipv6_collision = not self.frontends1And2HaveDifferentIPv6()
    del ipv6_collision  # a clean upgrade must converge; failures are not masked

    # Pre-upgrade: the production frontend runs the OLD SR and serves traffic.
    # caddy-frontend-2 (OLD) is the frontend bound to the test IPv4; the NEW
    # caddy-frontend-1 is stopped so it does not contend for the HTTPS port.
    self._requestMixedCluster('stopped', 'started')
    self.slap.waitForInstance(self.instance_max_retry)
    self._assertControlPlaneRunsNewSoftwareRelease()
    ...
    # Upgrade in place: move caddy-frontend-2 from the OLD SR to the NEW SR under
    # test, keeping it the frontend that serves. The cluster must converge and
    # keep delivering traffic -- a successful direct upgrade from the OLD release.
