...
Slaves default to ``normalize-accept-encoding: true``: the client-facing haproxy collapses the long tail of browser-specific ``Accept-Encoding`` strings to a small canonical set before forwarding to the origin. This replaces the gzip-only ``prefer-gzip-encoding-to-backend`` parameter from earlier releases. Set ``normalize-accept-encoding: false`` on a slave to forward the client's ``Accept-Encoding`` verbatim.
...
``enforced-compression`` is exposed at both the cluster (master) and slave level with the same name. Allowed values are ``none``, ``deflate``, ``gzip``, ``br``, ``zstd``; the default is ``gzip``. Each slave inherits the cluster default and may override; the cluster value is **not** a ceiling -- the slave is free to pick a stronger or weaker encoding, or ``none`` to opt out for that slave.

Trigger
"""""""

When the client sends no ``Accept-Encoding`` header at all, or sends the explicit wildcard ``Accept-Encoding: *`` -- both of which mean "any encoding is acceptable" per RFC 9110 §12.5.3 -- the client-facing haproxy substitutes the canonical form for the slave's effective ``enforced-compression`` value before forwarding upstream. When the client sent any concrete ``Accept-Encoding`` (including ``identity`` or an unknown coding), the rewrite does nothing.
