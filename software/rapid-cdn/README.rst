...
In order to being able to support some backward compatiblity those parameters are hidden from `software.cfg.json <software.cfg.json>`_:

 * ``-compatibility-node-reference``, ``type``: ``string``, which allows to set instance title during the request, so that old style clusters can be upgraded, typical value is ``caddy-frontend-N``, where ``N`` is number of the node in the cluster
