...
Codes 400, 404, 408, and 500 describe states of the CDN itself rather
than of any particular hosted site, so only the operator can customise
them.  Codes 502, 503, and 504 describe states of a hosted site's
backend; the operator sets the cluster default and each site owner can
override it for their own site.
...
.. list-table::
   ...
   * - Code
     - When the CDN emits this code
     - Who can customise

   * - 400
     - **Bad Request** -- the frontend HAProxy could not parse the
       incoming HTTP request.  Typical triggers: malformed request
       line (invalid method, URI, or HTTP version); missing or
       malformed ``Host`` header in HTTP/1.1; request line or headers
       exceeding the buffer (Rapid.CDN sets ``tune.bufsize`` to
       64 KiB); invalid characters or framing in headers.  Routing
       has not happened yet at this point, so the operator's
       cluster-default page always applies.
     - Operator

   * - 404
     - **Not Found** -- the request's ``Host`` header did not match
       any shared instance configured in this cluster.  The frontend
       HAProxy routes the request to ``BACKEND_NOT_FOUND``, which
       serves the operator's 404 page.  A 404 produced *by* a
       backend (a site responding with 404 for an unknown path on
       its own service) is forwarded to the client unchanged; only
       "unknown domain" 404s come from the EPM.  Site owners cannot
       override this code because, by definition, the cluster has no
       site to attribute the request to.
     - Operator

   * - 408
     - **Request Timeout** -- the client opened a connection but did
       not send a complete request within the cluster's request
       timeout (configurable per cluster, default 12 s).  Typical
       causes: slow or stalled client networks; clients that
       complete the TLS handshake but never send the HTTP request;
       scanners / probes holding sockets open without writing.  No
       site has been identified at the point of the timeout, so this
       code is operator-only.
     - Operator

   * - 500
     - **Internal Server Error** -- the CDN infrastructure itself
       (HAProxy or the SR's own wiring) failed to process the
       request.  Rare in practice; usually points at a bug or
       operational issue inside the CDN.  If a *backend* responds
       with 500, the CDN forwards it as-is and the EPM is not
       involved -- only CDN-internal 500s reach the operator page.
     - Operator

   * - 502
     - **Bad Gateway** -- the backend HAProxy reached the backend
       server, but the response is unparseable: malformed status
       line, truncated headers, framing errors, unexpected protocol
       downgrades.  The TCP connection succeeded; the HTTP exchange
       did not.  Site owners often override this page to brand the
       message for their own audience -- the override applies to
       their site only.
     - Operator + Site owner

   * - 503
     - **Service Unavailable** -- the backend HAProxy has no healthy
       backend to serve the request.  Common causes: the configured
       backend rejected the connection or timed out before
       responding (within ``backend-connect-timeout``); active
       health-checks marked all backends down; the shared instance
       is configured without a ``url`` (the degenerate case -- the
       CDN still serves a 503 page rather than failing unhandled).
       The most frequently customised code: this is the "we'll be
       right back" page visitors see during maintenance windows or
       backend outages.
     - Operator + Site owner
   ...
   * - 504
     - **Gateway Timeout** -- the backend connection was established
       ...
       produce a complete response within ``request-timeout``
       (default 12 s in Rapid.CDN).  Typical causes: overloaded
       backends, slow upstream database queries, hanging worker
       processes.
     - Operator + Site owner
