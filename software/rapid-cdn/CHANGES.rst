...
The changelog covers the CDN frontend Software Release across its whole
history and renames: **rapid-cdn** from 1.0.309 (2023) onward,
previously **caddy-frontend** for 1.0.149–1.0.298 (2020–2023), which
itself began in 2018 as a copy of the original **apache-frontend**. The
``caddy-frontend`` → ``rapid-cdn`` change (1.0.309) was a pure rename;
the parameter schema files were renamed to their stable names one
release earlier, at 1.0.298 (see that entry). The release does not use
Semantic Versioning: it ships with the shared, monotonic SlapOS
``1.0.<n>`` tags. Every tag that reached the clusters is listed —
including ones later cancelled or reverted, marked as such, since they
are part of the history; tags with no Software Release change are
omitted.
...
Tag `1.0.240 <https://lab.nexedi.com/nexedi/slapos/-/tags/1.0.240>`__. Cancelled — never deployed.
