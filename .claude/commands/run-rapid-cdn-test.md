# Run rapid-cdn Integration Test

Run a rapid-cdn integration test in the background with proper environment setup.
...
- `integration-test-environment-script`: path to the `slapos-local-development-environment.sh` for integration tests
...
If a process is found, warn the user and do **not** start a new test. rapid-cdn tests
...
source <integration-test-environment-script> && \
...
Tell the user the test is running in the background and approximately how long it may take (see timing table below). When the background task completes, analyze the output.
