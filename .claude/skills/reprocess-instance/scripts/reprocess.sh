...
# Run slapos node instance. --only does not imply --force, so both flags
# are required to force reprocessing of a specific partition. --all is
# deprecated; use --force on its own to reprocess everything.
if [ -n "$PARTITION" ]; then
