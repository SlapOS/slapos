...
class TestSlapOSNodeBuild(SlapOSInstanceTestCase):
  ...
  def test(self):
    ...
    firewall_executable = os.path.join(build_directory, 'parts', 'firewalld', 'bin', 'firewall-cmd')
    self.assertTrue(os.access(firewall_executable, os.X_OK))
    with self.assertRaises(subprocess.CalledProcessError) as ctx:
      subprocess.check_output(
        [firewall_executable, '--version'],
        stderr = subprocess.STDOUT,
        universal_newlines = True
      )
    self.assertIn(
      'Failed to connect to socket /opt/slapos/parts/dbus/var/run/dbus/system_bus_socket',
