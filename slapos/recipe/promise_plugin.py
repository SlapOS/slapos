...
class Recipe(GenericBaseRecipe):
  ...
  def __init__(self, buildout, name, options):
    ...
    if klass == 'RunPromise':
      ...
      if not re.search(r"^[\w_]+$", klass):
        raise ValueError("%r is not valid class name" % klass)

    if bool(module) == bool(filepath):
      raise UserError("Either 'module' or 'file' is required but not both")

    if module:
      if not re.search(r"^[\w_\.]+$", module):
        raise ValueError("%r is not a valid module qualified name" % module)
      if klass:
        content = 'from %s import %s as RunPromise' % (module, klass)
      else:
        content = 'from %s import RunPromise' % module
    else:
      content = 'exec(%r)' % filepath
      if klass:
        content += '\n\nRunPromise = ' + klass
