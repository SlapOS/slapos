...
class JsonSchema(Recipe):
  ...
  def _getSharedSchema(self, software_schema):
    ...
    schema =  software_schema.getSoftwareSchema()
    for subschema in schema['software-type'].values():
      if subschema['software-type'] == t and subschema.get('shared') == True:
        url = urljoin(software_schema.software_url, subschema['request'])
        return _readAsJson(url, True)

  def _validate(self, validator, instance):
  ...
  def _parseParameters(self, software_schema, parameter_dict):
    ...
    self._validate(validator, instance)
    return instance

  def _parseSharedParameters(self, shared_validator, shared_dict):
    instance = shared_dict if isinstance(shared_dict, dict) else {}
    self._validate(shared_validator, instance)
    return instance
  ...
  def _expandParameterDict(self, options, parameter_dict):
    ...
    if options['slave-instance-list']:
      shared_schema = self._getSharedSchema(software_schema)
      shared_validator = DefaultValidator(shared_schema)
      options['slave-instance-list'] = [
        self._parseSharedParameters(shared_validator, shared_dict)
        for shared_dict in options['slave-instance-list']
      ]
    parameter_dict = self._parseParameters(software_schema, parameter_dict)
    options['configuration'] = parameter_dict
    return parameter_dict
