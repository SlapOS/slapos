...
class DefaultValidator(object):
  ...
  def descend(self, kls):
    # Route Validator.descend through evolved.iter_errors(instance) so the
    # scope-tracking wrapper installed by `iter_errors` above observes every
    # descended sub-schema (otherwise only the top-level call is observed and
    # `applied_schemas` stays empty for nested schemas, defeating the
    # collect-defaults / unstringify logic). On jsonschema < 4.18, descend
    # already calls iter_errors internally; returning None here skips the
    # patch in `create`.
    if not _USE_REFERENCING:
      return None
...
class JsonSchemaWithDBFromInstanceNode(JsonSchemaWithDB):
  ...
  All instances (both valid and invalid) are loaded from the database.
  """
  ...
  def _expandParameterDict(self, options, parameter_dict):
    ...
    # All instances (both valid and invalid) are loaded.
    slave_instance_list = self._load_valid_instances_from_db(
      db_path, valid_only=False)
