...
class JsonSchema(Recipe):
  ...
  def _parseSharedParameters(self, software_schema, options):
    shared_list = options['slave-instance-list']
    if not shared_list:
      return
    ...
    options['slave-instance-list'] = valid
  ...
  def _expandParameterDict(self, options, parameter_dict):
    ...
    self._parseSharedParameters(software_schema, options)
