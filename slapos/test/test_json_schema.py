...
def createInstanceParameterSchemaValidatorTest(path):
  # Test that json is a valid json schema, supports only latest validator
  # to force usage of the latest schema everywhere.
  validator_dict = {
...
def generateSoftwareCfgTest():
  ...
  for path in glob.glob("%s/software/*/software*.cfg.json" % base_path):
    test_name = "test_%s" % '_'.join(path.split("/")[-2:]).replace('.', '_')
